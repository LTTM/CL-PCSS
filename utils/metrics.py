...
class Metrics:
    def __init__(self, name_classes, mask_unlabeled=False, mask=None, log_colors=True, device='cuda'):
    ...
    def __str__(self):
        ...
        if self.mask_unlabeled:
            mpa, mpp, miou = self.nanmean(pa[1:]), self.nanmean(pp[1:]), self.nanmean(iou[1:])
            spa, spp, siou = self.nanstd(pa[1:]), self.nanstd(pp[1:]), self.nanstd(iou[1:])
        else:
