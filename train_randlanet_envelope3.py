...
if __name__ == '__main__':
    ...
    if CL:
        ...
        logdir = "log/train" + "_step" + str(lstep) + "_Inpaint_" + setup + "_" + datetime.datetime.now().strftime("%H-%M-%S") + pretrain
    else:
    ...
    rset = dataset(pointcloud_dataset=SemanticKITTIDataset(split='train',
                                                           ues=False,
                                                           CL=CL,
                                                           c2f=c2f,
                                                           step=lstep-1,
    ...
    for e in range(epochs):
        ...
        for i, data in enumerate(pbar):
            ...
            #reg_loss = mse(f, f_s)
            #y_s = o_s.argmax(dim=2)
            yy = rset[i]["labels"].cpu().numpy()
            mask = torch.where(yy>0)
            y[mask] = -2 #y_s[mask]
