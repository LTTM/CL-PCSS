...
step_0_classes = [9,10,11,12,15,17]
step_1_classes = [13,14,16,18,19]
step_2_classes = [1,2,3,4,5,6,7,8]
...
if __name__ == '__main__':
    ...
    val_cube_edge = 96
    num_classes = 20
    num_prev_classes = 7 #20
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    dataset = SemanticKITTIDataset

    if CL:
        fine1 = ""#"step1_L1_C2F_18-46-39step2_L1_Sequential_Masked_13-04-07step2_L1_Sequential_Masked_12-27-31step1_L1_Sequential_Masked_09-14-52step0" #semantic
        #fine2 = "step1_c2f_Coarse-to-fine-" #geomentric
        pretrain = ""
    ...
    """
    old_model = Cylinder3D(num_prev_classes)  # SimpleNet(num_classes)
    old = torch.load("log/train_" + fine1 + "/val_best.pth")
    old_model.load_state_dict(old)
    old_model.to('cuda')
    old_model.eval()
    """

    #old_model2 = RandLANet(num_neighbors=16, device='cuda',
    #                      num_classes=7)  # SimpleNet(num_classes)
    #old = torch.load("log/train_" + fine1 + "/val_best.pth")
    #old_model2.load_state_dict(old)
    #old_model2.to('cuda')
    #old_model2.eval()
    ...
    #old_model2 = RandLANet(num_neighbors=16, device='cuda',
    #                  num_classes=7)  # SimpleNet(num_classes)
    # = torch.load("log/train_" + fine2 + "/val_best.pth")
    #old_model2.load_state_dict(old)
    #old_model2.to('cuda')
    #old_model.eval()
    ...
    vset = dataset(augment=False, split='val',
                                                           CL=CL,
                                                           c2f=True,
                                                           step=lstep,
                                                           setup=setup)
    ...
    for e in range(epochs):
        ...
        if e % 1 == 0:
            ...
            metrics = Metrics(vset.cnames, device=device, mask=True)
        ...
        for i, data in enumerate(pbar):
            ...
            loss = nn.CrossEntropyLoss(ignore_index=-1)
                                       #weight=torch.tensor(dset.pointcloud_dataset.weights,
                                       #                    dtype=torch.float32,device=device))
            loss_kd = nn.CrossEntropyLoss(ignore_index=-1)
            mse = nn.L1Loss()
            #kd = KnowledgeDistillationLoss()
            ...
            optim.param_groups[0]['lr'] = lr / 10
            lr = adjust_learning_rate(optim, lr_decays, e + 30*lstep)
            optim.zero_grad()
            ...
            #o_s = old_model(data)
            y_s = torch.from_numpy(step_map.__getitem__(data["labels"])).cuda()
            #sem = old_model(data)
            ...
            #idmap = {k: v for k, v in dset.pointcloud_dataset.idmap.items() if v in np.unique(step_map)}
            #o_g = old_model(data)
            #y_g = torch.from_numpy(step_map.__getitem__(data["labels"])).cuda()
            #geo = old_model2(data)
            #envelope = nn.functional.softmax(sem + eo, dim=2)
            #envelope = envelope.argmax(dim=2)
            #old_ce = mse(o, envelope)
            #old_ce_sem = mse(o_s, torch.nn.functional.one_hot(y_s).float())

            #mask = y>0
            #nmask = mask==False
            """
            o_s, f_s = old_model(data)
            model.eval()
            old_model.train()
            old_ce = loss(o_s.swapaxes(1, 2), y_s)
            old_ce.backward()
            old_model.eval()
            model.train()
            """
            ...
            o, f = model(data)
            y = data["labels"].cuda()
            old_ce_sem = - (1/len(dset)) * loss_kd(o.swapaxes(1,2), o_s.swapaxes(1,2))#.argmax(dim=2))
            #old_ce_geom = loss_kd(o.swapaxes(1, 2), o_g.argmax(dim=2))
            #reg_loss = mse(f, f_s)

            l = loss(o.swapaxes(1,2), y) #+ 0.1 * old_ce_sem #0.1 * reg_loss #
            l.backward()
