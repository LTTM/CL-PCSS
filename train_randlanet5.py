...
from utils.my_losses import RandLANetContrastiveLoss
...
if __name__ == '__main__':
    ...
    if CL:
        ...
        logdir = "log/train" + "_step" + str(lstep) + "_contrastive_" + setup + "_" + datetime.datetime.now().strftime("%H-%M-%S") + pretrain
    else:
    ...
    for e in range(epochs):
        ...
        for i, data in enumerate(pbar):
            ...
            contrastive_loss = RandLANetContrastiveLoss(num_classes=num_classes)
            ...
            ########################################################################
            weights = torch.from_numpy(np.load("m.npy")).cuda()
            class_weights = weights[y]
            geom_contrastive = contrastive_loss(o, y, class_weights=class_weights)

            l = loss(o.swapaxes(1, 2), y) + 0.5 * geom_contrastive # if step < tot_steps//2 else loss2(o, y)
            l.backward()
