...
if __name__ == '__main__':
    ...
    if CL:
        pretrain = "step1_NEW_Disjoint"
        #pretrain = "step"+str(step-1) #"step0__18-54-36" #pcs_semantic-kitti_randlanet_bis"  # "s3dis"
        logdir = "log/train" + "_step" + str(lstep) + "_NEW_" + setup + "_" + datetime.datetime.now().strftime("%H-%M-%S") + pretrain
    else:
    ...
    old_model = RandLANet(num_neighbors=16, device='cuda',
                      num_classes=20)  # SimpleNet(num_classes)
    old = torch.load("log/train_" + pretrain + "/val_best.pth")
    ...
    for e in range(epochs):
        ...
        for i, data in enumerate(pbar):
            ...
            old_ce = loss(oo.swapaxes(1,2), yo)
            ########################################################################
            """
