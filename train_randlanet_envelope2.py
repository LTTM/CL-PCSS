...
if __name__ == '__main__':
    ...
    if CL:
        fine1 = "step1_KD-L2_Sequential_Masked_11-18-53step0" #semantic
        fine2 = "step1_c2f_Coarse-to-fine-" #geomentric
        pretrain = "step2_Both_Sequential_17-36-33step2_KD-L2_Sequential_Masked_16-40-31step1_KD-L2_Sequential_Masked_11-18-53step0"
        #pretrain = "step"+str(step-1) #"step0__18-54-36" #pcs_semantic-kitti_randlanet_bis"  # "s3dis"
        logdir = "log/train" + "_step" + str(lstep) + "_Both_" + setup + "_" + datetime.datetime.now().strftime("%H-%M-%S") + pretrain
    else:
    ...
    old_model = RandLANet(num_neighbors=16, device='cuda',
                      num_classes=num_prev_classes)  # SimpleNet(num_classes)
    ...
    for e in range(epochs):
        ...
        for i, data in enumerate(pbar):
            ...
            loss_kd = nn.CrossEntropyLoss(ignore_index=-1, reduction='mean')
            mse = nn.MSELoss()
            ...
            o, f = model(data)
            y = data["labels"].cuda()
            old_ce_sem = loss_kd(o.swapaxes(1,2), o_s.argmax(dim=2))
            #old_ce_geom = loss_kd(o.swapaxes(1, 2), o_g.argmax(dim=2))
            reg_loss = mse(f, f_s)

            l = loss(o.swapaxes(1,2), y) + 0.1 * (old_ce_sem + reg_loss)
            l.backward()
