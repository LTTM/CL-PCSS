...
from utils.S3DISdataset import S3DISDataset
...
if __name__ == '__main__':
    ...
    dset = dataset(pointcloud_dataset=S3DISDataset(split='train'),
                                                           model = my_model)
    ...
    vset = dataset(pointcloud_dataset=S3DISDataset(augment=False, split='val'),
    ...
    for e in range(epochs):
        ...
        for i, data in enumerate(pbar):
            ...
            fair_loss = LEAK.FairLoss(alpha=10., mapping=DATA["micro2macro_S3DIS"],
                                      idx=DATA["macro_idx_S3DIS"], classes=num_classes)
            ...
            macro_prototypes = torch.zeros((len(DATA["macro_idx_S3DIS"]), 512)).cuda()
            K = torch.zeros((num_classes)).cuda()
            M = torch.zeros((len(DATA["macro_idx_S3DIS"]))).cuda()
