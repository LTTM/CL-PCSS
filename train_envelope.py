...
def main(args):
    ...
    if args.CLstrategy == "okd":
        print("using output knowledge distillation")
    elif args.CLstrategy == "fkd":
        print("using feature knowledge distillation")
    elif args.CLstrategy == "inpaint":
    ...
    if args.pretrained_model:
        ...
        if args.CLstep != 0:
            old_model = RandLANet(num_neighbors=16, device='cuda', num_classes=len(previous_step_classes))
    ...
    for e in range(args.epochs):
        ...
        for i, data in enumerate(pbar):
            ...
            mse = nn.MSELoss()
            loss_kd = nn.CrossEntropyLoss(ignore_index=0)
            ...
            # separate current and previous labels
            if args.CLstep != 0 and args.CLstrategy == 'inpaint':
                mask_s = [t for t,s in enumerate(y) if (s < len(previous_step_classes))]
                # mask_c = [t for t,s in enumerate(y) if s==0 or len(previous_step_classes) <= s < len(current_step_classes)]
                # y = np.array(y)                      
                # y_c = y[mask_c]
                # y_s = y[mask_s]
            ...
            if args.CLstep != 0:
                ...
                if args.CLstrategy == 'kd':
                    old_ce_sem = - (1/len(dset)) * loss_kd(o.swapaxes(1,2).contiguous(), o_s.argmax(dim=2))
                    reg_loss = (1/len(dset)) * mse(f, f_s)
                    l += args.kd_par * old_ce_sem + args.kd_par * reg_loss
                elif args.CLstrategy == 'okd':
                    old_ce_sem = - (1/len(dset)) * loss_kd(o.swapaxes(1,2).contiguous(), o_s.argmax(dim=2))
                    l += args.kd_par * old_ce_sem
                elif args.CLstrategy == 'fkd':
                    reg_loss = (1/len(dset)) * mse(f, f_s)
                    l += args.kd_par * reg_loss   
            l.backward()
            ...
            # inpainting
            if args.CLstep != 0 and args.CLstrategy == 'inpaint':
                pred_s = o_s.argmax(dim=2).flatten()
                pred[mask_s] = pred_s[mask_s]
